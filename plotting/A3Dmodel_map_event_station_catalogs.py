...
def plot_stations(stations, ax):    
    ax.scatter(stations["lon"], stations["lat"], marker=".", ec="darkgreen", c="#87d768", s = 40, transform = ccrs.PlateCarree(), label = "stations", zorder=10)
...
if __name__ == "__main__":
    ...
    ev_mw, ev_depth = [],[]
    for ev in events:
        ev_depth.append(ev.preferred_origin().depth/1000)
    ...
    lat0,lon0 = compute_mean_point_stations_event(events[0],stations)
    ...
    cax = dax.new_horizontal(size="2%", pad=0.05, axes_class=plt.Axes)
    fig.add_axes(cax)
