...
class WaveformPlotting(object):
    ...
    def __init__(self, **kwargs):
        ...
        # Cmap to use
        self.cmap = kwargs.get("cmap", False)    
    ...
    def __sect_init_color(self):
        ...
        elif self.color == 'distance':
            colors = {_tr.stats.distance/1000.0 for _tr in self.stream}
        else:
            self.sect_color = self.color
            return
                
        cmap = get_cmap('Paired', lut=len(colors)) if not(self.cmap) else self.cmap
        
        # self.sect_color = {k: cmap(i) for i, k in enumerate(sorted(colors))}
        self.sect_color = {k*1000: cmap(k) for k in colors}
